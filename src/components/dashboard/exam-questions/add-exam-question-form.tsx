...
export function AddExamQuestionForm({
  onQuestionAdded,
}: {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `examQuestions/${user.uid}/${Date.now()}-${file.name}`
      );
      ...
      const newQuestionRef = doc(collection(db, 'examQuestions'));
      ...
      toast({
        title: 'Exam Questions Submitted',
        description:
          'Your questions have been sent to the Exam Officer for review.',
      });
      form.reset();
      fileRef.current.value = ''; // Clear the file input
      onQuestionAdded();
    } catch (e: any) {
      console.error('Error submitting exam questions: ', e);
      toast({
        variant: 'destructive',
        title: 'Submission Failed',
        description: e.message || 'There was a problem submitting your file.',
      });
    } finally {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Title / Exam Type</FormLabel>
              <FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Question Document</FormLabel>
              <FormControl>
                <Input
                  type="file"
                  accept=".pdf, .doc, .docx"
              ...
              <FormDescription>
                Upload your questions in PDF or Word format.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
